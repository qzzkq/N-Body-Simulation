...
Проект умеет:
- загружать начальные условия из **HDF5** (`.h5`) **TXT** (.txt) или генерировать систему случайно;
...
координаты — А.Е (астрономическая единица)

скорость — А.Е в год

масса — M (масса солнца)
...
| ----------- | ---------: | ---------: | ------------------------------------------------------------------ |
| Brute Force |        CPU |      O(N²) | RK4 (Runge-Kutta)|
...
for_clean_sys    исполняемый файл с необходимыми библиотеками для запуска на чистой машине
project/
  algo/          алгоритмическое ядро проекта
